...
export function matchWhenForTeam(segments, when) {
  ...
  if ("modules" in when && Array.isArray(when.modules)) {
    const mods = normModuleList(when.modules);
    const mk = String(when.match || "")
      .trim()
      .toLowerCase();
    if (mods.length && mk === "first") return mods.includes(segL[0]);
    if (mods.length && mk === "any") return segL.some((s) => mods.includes(s));
